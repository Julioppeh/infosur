...
def create_app() -> Flask:
    ...
    app.config["BUILD_NUMBER"] = os.environ.get("INFO_SUR_BUILD_NUMBER") or datetime.utcnow().strftime(
        "%Y%m%d%H%M%S"
    )
    ...
    def editor() -> str:
        return render_template(
            "editor.html", article_fields=ARTICLE_FIELDS, build_number=app.config["BUILD_NUMBER"]
        )
    ...
    def api_create_article():
        ...
        satire_raw = data.get("satire_level", 50)
        try:
            satire_level = int(satire_raw)
        except (TypeError, ValueError):
            satire_level = 50
        satire_level = max(0, min(100, satire_level))
